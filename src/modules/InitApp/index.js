...
import { encodeToHex, decodeFromHex } from '../../utils/hexUtils';
...
class InitApp {
  constructor(props) {
    ...
    this.WEBSITE_ADDRESS = 'http://localhost:3000';
  ...
  async _listeningPrivateChatMsg() {
    console.log('start listening');
    if (!localStorage.getItem('userInfo')) return;
    const userInfo = JSON.parse(localStorage.getItem('userInfo'));
    const filter = {
      topics: [encodeToHex('chainchat').substring(0, 10)],
      privateKeyID: userInfo.asymKeyId,
    };
      ...
      setInterval(() => {
        this.shh.getFilterMessages(filterId).then(messages => {
          ...
          for (const msg of messages) {
            const data = JSON.parse(decodeFromHex(msg.payload));
            console.log(data);
  ...
  _listeningInitMessage() {
    // 去后端拿
    let localPrivateChat = new Map();
    if (localStorage.getItem('privateChatList')) {
      localPrivateChat = new Map(JSON.parse(localStorage.getItem('privateChatList')));
    }
    // const groupChat = new Map(allMessage.groupChat);
    // store.dispatch(setHomePageListAction(allMessage.homePageList));
    store.dispatch(
      setAllPrivateChatsAction({
        data: localPrivateChat,
      }),
    );
    // store.dispatch(
    //   setAllGroupChatsAction({
